...
'''
        ...
        hz = torch.sum(self.embedding(inputs) * mask, -2) / torch.sum(mask, 1)
        concat = torch.cat([hidden.unsqueeze(1).repeat(1,self.opt.pos_num,1,1), pos_emb.unsqueeze(0).repeat(batch_size,1,1,1)], -1).sum(-2) / len #mask.squeeze(-1).sum(-1).view(batch_size, 1, 1)
