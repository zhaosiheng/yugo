...
'''
        ...
        h = torch.matmul(self.leakyrelu(torch.matmul(torch.log2(log)+1, self.Q)), self.P) #.sum(-2) / torch.sum(mask, 1)
        ...
        #nh = torch.tanh(nh)
        nh = pos_emb + hidden
        zr = nh[torch.arange(nh.shape[0]).long(), torch.sum(mask, 1).long() - 1, torch.arange(nh.shape[2]).long()]
        nh = torch.sigmoid(self.glu1(nh) + self.glu2(hs))
        beta = torch.matmul(nh, self.w_2)
        beta = beta * mask
        select = torch.sum(beta * hidden, 1)
       
        #select = torch.matmul(torch.cat([select, zr.squeeze(-2)], -1), self.yogo)
        
        print(select.shape)
        print(zr.squeeze(-2).shape)
