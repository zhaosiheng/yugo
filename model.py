...
class CombineGraph(Module):
    def __init__(self, opt, num_node, adj_all, num):
        ...
        self.Q = nn.Parameter(torch.Tensor(self.dim+1, self.dim+1))
        self.P = nn.Parameter(torch.Tensor(self.dim+1, opt.pos_num))
    ...
    def compute_scores(self, hidden, mask, inputs, epoch):
        ...
        '''(3)'''
        pos_emb = self.pos_emb[:, :len, :].unsqueeze(0).repeat(batch_size, 1, 1, 1)
        h = torch.matmul(self.leakyrelu(torch.matmul(torch.cat((hs, torch.log(mask.squeeze(-1).sum(-1).unsqueeze(-1)), -1), 2), self.Q)), self.P)
