...
class GlobalAggregator(nn.Module):
    ...
    def forward(self, self_vectors, neighbor_vector, batch_size, masks, neighbor_weight, extra_vector=None):
        if extra_vector is not None:
            ...
            alpha = torch.matmul(torch.cat([extra_vector.unsqueeze(2).repeat(1, 1, neighbor_vector.shape[2], 1)*neighbor_vector, self_vectors.unsqueeze(2).repeat(1, 1, neighbor_vector.shape[2], 1)*neighbor_vector], -1), self.w_1).squeeze(-1)
